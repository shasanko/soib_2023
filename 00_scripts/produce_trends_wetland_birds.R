...
new_method <- read.csv("01_analyses_full/results/new_method_results (old and new).csv",
                       header = T)
...
wetland_species <- new_method %>% filter(COMMON.NAME %in% wetland_species) %>%
  pull(COMMON.NAME) %>% unique()
...
load("01_analyses_full/dataforanalyses_pre_centroid.RData")
...
totalspecieslist <- totalspecieslist %>% filter(COMMON.NAME %in% wetland_species)
...
totalspecieslist_ordered = specieslist %>%
  filter(COMMON.NAME %in% wetland_species) %>%
  dplyr::select(COMMON.NAME) %>%
  filter(COMMON.NAME %in% totalspecieslist$COMMON.NAME) %>%
  left_join(totalspecieslist)
