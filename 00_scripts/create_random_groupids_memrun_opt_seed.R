...
#   next_idx <- 1
#   done_count <- 0
#   while((next_idx<=1) || (done_count<1)) {
#     started <- 0
#     while((running < cores) && (next_idx<=1)) {
...
if (to_run == TRUE) {
  ...
  rgids_temp <- rgids[,1:2]
  
  rtable_1 <-locs %>%
    slice_sample(n = 1000, replace=TRUE) %>%
    ungroup()
  # rearrange the 1000 to represent each run
  rgids_1 <- matrix(rtable_1$group.id, ncol=1000, byrow=TRUE)
  rgids_1_temp <- rgids_1[,1:2]
