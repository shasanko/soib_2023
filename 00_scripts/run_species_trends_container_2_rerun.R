...
if (to_run == TRUE) {
  ...
  #            recursive = T)
  # }
  # if (!dir.exists("01_analyses_full/trends_rerun/")) {
  #   dir.create(cur_metadata$TRENDS.PATHONLY, 
  #              recursive = T)
  # }
  ...
  for (k in cur_assignment)
  {
    ...
    if(container) {
      ...
      trends_species_dir_temp <- glue("01_analyses_full/trends/trends_rerun/species_{k}")
      trends_species_dir <- as.character(trends_species_dir_temp)
      # TRENDS.PATHONLY <-  
      # trends_species_dir <- "01_analyses_full/trends_rerun/species_{k}"
      # trends_species_dir <- cur_metadata %>%
      #   dplyr::summarise(TRENDS.PATH = glue("{TRENDS.PATHONLY}/species_{k}")) %>%
      #   as.character()
    }
    trends_stats_dir_temp <- glue("01_analyses_full/trends/trends_rerun/species_{k}/stats")
    trends_stats_dir <- as.character(trends_stats_dir_temp)
    #trends_stats_dir <- "01_analyses_full/trends_rerun/species_{k}/stats"
    ...
    if(container) {
      ...
      write_path_temp <- glue("01_analyses_full/trends/trends_rerun/trends_{k}.csv")
      write_path <- as.character(write_path_temp)
      # write_path <- cur_metadata %>%
