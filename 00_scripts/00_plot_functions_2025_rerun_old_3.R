...
to_walk <- function(.x,
  ...
  # 
  # logo_img_width  <- ncol(stamp)
  # logo_img_height <- nrow(stamp)
  # 
  # logo_width <- 0.50 * x_range
  # #logo_width <- 1.2
  # logo_height <- logo_width * (logo_img_height / logo_img_width)
  ...
  if (cur_metadata$MASK.TYPE == "country") {
    cur_plot <- plot_base +
      ...
      annotation_raster(stamp,
                        ...
                        xmax = bb["xmax"] - pad_country * x_range,
                        ymin = bb["ymax"] - h * y_range,
                        ymax = bb["ymax"] - pad_country * y_range)
  ...
  if (cur_metadata$MASK.TYPE == "state") {
    cur_plot <- plot_base +
      ...
      #         linewidth = 1)+
      # annotation_raster(stamp,
      #                   xmin = bb_pad["xmax"] - logo_width,
      #                   xmax = bb_pad["xmax"],
      #                   ymin = bb_pad["ymax"] - logo_height,
      #                   ymax = bb_pad["ymax"]) +
      annotation_raster(stamp,
                        xmin = bb_pad["xmax"] - logo_width,
                        xmax = bb_pad["xmax"],
...
soib_rangemap <- function(which_spec = "all", cur_mask = "none") {
  ...
  if (cur_metadata$MASK.TYPE == "country") {
    ...
    admin_sf <- dists_sf %>% dplyr::filter(STATE.NAME == cur_mask)
    cur_state_sf <- states_sf %>% dplyr::filter(STATE.NAME == cur_mask)
    
    load("00_data/grids_st_sf.RData")
    # This loads data frames, not sf objects. 
    rm(g2_st_sf, g3_st_sf, g4_st_sf)
    g1_st_sf_1 <- st_sf(g1_st_sf, sf_column_name = "GEOM.G1") # This has to be converted to an sf object
    cur_g1_sf <- g1_st_sf_1 %>% dplyr::filter(STATE.NAME == cur_mask)
    ...
    #logo_width <- 0.10 * x_range
    logo_width <- 0.5
    logo_height <- logo_width * (logo_img_height / logo_img_width)
    
    # pad_frac_x <- 0.4  # 40% padding in x
    # pad_frac_y <- 0.1 # 10% padding in y
    
    # Create a padded canvas from bb to accommodate the soib logo
    bb_pad <- bb
    # bb_pad["xmin"] <- bb["xmin"] - pad_frac_x * x_range
    # bb_pad["xmax"] <- bb["xmax"] + pad_frac_x * x_range
    # bb_pad["ymin"] <- bb["ymin"] - pad_frac_y * y_range
    # bb_pad["ymax"] <- bb["ymax"] + pad_frac_y * y_range
    # 
    # bb_pad["xmin"] <- bb["xmin"] - 0.85
    # bb_pad["xmax"] <- bb["xmax"] + 0.85
    # bb_pad["ymin"] <- bb["ymin"] - 0.7
    # bb_pad["ymax"] <- bb["ymax"] + 0.7
    
    # bb_pad["xmin"] <- bb["xmin"] - 1.2*logo_width
    # bb_pad["xmax"] <- bb["xmax"] + 1.2*logo_width
    # bb_pad["ymin"] <- bb["ymin"] - 1.2*logo_height
    # bb_pad["ymax"] <- bb["ymax"] + 1.2*logo_height
    
    bb_pad["xmin"] <- bb["xmin"]
    bb_pad["xmax"] <- bb["xmax"]
    bb_pad["ymin"] <- bb["ymin"]
    bb_pad["ymax"] <- bb["ymax"] + 1.01*logo_height
  ...
  if (any(!cur_spec %in% specieslist$COMMON.NAME)) {
    ...
    if (advanced_kenbunshoku) {
      ...
      # future_walk(cur_spec, .progress = TRUE, ~ to_walk(.x,
      #                                                   occ_final,
      #                                                   vagrant_presence,
      #                                                   cur_g1_sf,
      #                                                   admin_sf,
