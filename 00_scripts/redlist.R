...
# Read latest BLI gen values
threegen <- read.csv("00_data/genbli.csv") # This file contains generation lengths of species from BLI.
...
# I don't understand the following command. GEN is already 3 generations
soib <- soib %>%
  mutate(
...
redlist <- soib %>%
  ...
  mutate(
    ...
    Decline3GEN = ifelse(valid3GEN,
                         ...
                         NA),    Years1GEN = GEN1,
    Decline3GENMean = ifelse(valid3GEN,
                         round(100 * (1 - (1 + currentslopemean/100)^GEN3),0),
                         NA),    Years1GEN = GEN1,
    Decline3GENLCI = ifelse(valid3GEN,
                         round(100 * (1 - (1 + currentslopelci/100)^GEN3),0),
                         NA),    Years1GEN = GEN1,
    Years2GEN = GEN2,
    Years3GEN = GEN3,
...
colnames(redlist) <- c(
  "Species", # India Checklist Common Name
  "Range Coverage",
  "Mean Grid Coverage",
  "Long-term Decline",
