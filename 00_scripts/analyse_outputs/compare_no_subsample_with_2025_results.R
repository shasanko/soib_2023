...
# old <- old %>% filter(COMMON.NAME %in% c(unique(new$COMMON.NAME)))
...
species_set <- "Black Drongo"

label_data <- trend_summary %>%
  filter(COMMON.NAME == "Black Drongo") %>%
  dplyr::select(COMMON.NAME, mean_pct_diff, width_ratio)

plot_data_list <- lapply(species_set, function(sp) {
  sp_data <- combined %>% filter(COMMON.NAME == "Black Drongo")
